...
void test_fuege_kfz_hinzu_belegt(){
    ...
    p_parkhaus->parkplaetze[0] = init_kfz(p_parkhaus, 1, 60);
    p_parkhaus->parkplaetze[0]->belegt = 1; //Belegt den ersten Parkplatz
    p_parkhaus->parkplaetze[1] = init_kfz(p_parkhaus, 2, 60);
...
void test_entferne_kfz_normalfall(){
    ...
    p_parkhaus->p_parkplaetze[0] = init_kfz(p_parkhaus, 2, 60);
    p_parkhaus->p_parkplaetze[0]->belegt = 1;
    p_parkhaus->p_parkplaetze[1] = init_kfz(p_parkhaus, 3, 60);
    p_parkhaus->p_parkplaetze[1]->belegt = 1;
    p_parkhaus->p_parkplaetze[2] = p_kfz;
    ...
    entferne_kfz(p_parkhaus, p_kfz); //Entfernt das Kfz aus dem Parkhaus
    
    assert(p_parkhaus->p_parkplaetze[2].belegt == NULL); //Überprüft, ob der Parkplatz des Kfz jetzt frei ist
...
void test_entferne_kfz_nicht_vorhanden(){
    ...
    assert(p_parkhaus->p_parkplaetze[0] != NULL); //Überprüft, ob der erste Parkplatz weiterhin belegt ist
    assert(p_parkhaus->p_parkplaetze[1] != NULL); //Überprüft, ob der zweite Parkplatz weiterhin belegt ist
