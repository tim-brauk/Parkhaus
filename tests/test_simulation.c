...
void test_simuliere_simuliere_zeitabschnitt_kfz_entfernt(){
    ...
    p_parkhaus->p_parkplaetze[0]->p_kfz->verbleibende_parkzeit = 0; //Setzt die verbleibende Parkzeit des Kfz auf 0, damit es entfernt wird
    int id = 4;
    int zeitpunkt = 40;
    simuliere_zeitabschnitt(p_parkhaus, p_simulationsparameter, NULL, &id, &zeitpunkt); //Simuliert einen Zeitabschnitt

    assert(p_parkhaus->p_parkplaetze[0] == NULL); //Überprüft, ob das Kfz mit der überschrittenen Parkzeit entfernt wurde
    assert(p_parkhaus->p_parkplaetze[1]->verbleibende_parkzeit == 14);
    assert(p_parkhaus->p_parkplaetze[2]->verbleibende_parkzeit == 24); //Überprüft, ob die verbleibende Parkzeit der anderen Kfz um 1 reduziert wurde
